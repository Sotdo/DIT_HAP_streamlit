...
# ================================= Main Functions =================================
# @st.cache_resource
def load_ontology_data(ontology_data: GeneOntologyData | FYPOData | MONDOData, **kwargs) -> tuple[GODag, GafReader, dict]:
    """Load ontology data from obo file and association file."""
    try:
        dag = GODag(str(ontology_data.ontology_obo), optional_attrs=['def', 'relationship'], load_obsolete=False)
    except KeyError:
        dag = GODag(str(ontology_data.ontology_obo), optional_attrs=['def'], load_obsolete=False)
...
# @st.cache_resource
def ontology_enrichment(query_genes: list[str], bg_genes: list[str], **kwargs) -> tuple[GOEnrichmentStudyNS, list[GOEnrichmentRecord]]:
...
def format_ontology_enrichment_results(label: str, oea_results_sig: list[GOEnrichmentRecord], **kwargs) -> pd.DataFrame:
    ...
    if oea_results_sig_prt.empty:
        ...
        oea_results_sig_prt["gene_ratio"] = round(oea_results_sig_prt["study_count"] / oea_results_sig_prt["study_n"], 2)
        oea_results_sig_prt["term_coverage"] = round(oea_results_sig_prt["study_count"] / oea_results_sig_prt["pop_count"], 2)

        # sort gene items
        oea_results_sig_prt["study_items"] = oea_results_sig_prt["study_items"].apply(lambda x: ",".join(sorted(x.split(", "))))
        oea_results_sig_prt["pop_items"] = oea_results_sig_prt["pop_items"].apply(lambda x: ",".join(sorted(x.split(", "))))
        ...
        kept_columns = ["NS", "GO", "name", "level", "depth", "p_uncorrected", "p_fdr_bh", "gene_ratio", "term_coverage", "study_count", "study_n", "pop_count", "pop_n", "ratio_in_study", "ratio_in_pop", "study_items", "pop_items"]
        if "defn" in oea_results_sig_prt.columns:
            kept_columns.append("defn")
        oea_results_sig_prt = oea_results_sig_prt[kept_columns].copy().rename(
            columns={
                "NS": "namespace",
                "GO": "term_id",
                "name": "term",
...
@st.cache_data
def ontology_enrichment_pipeline(ontology_data: GeneOntologyData | FYPOData | MONDOData, query_genes: list[str], bg_genes: list[str], load_kwargs: dict = {}, enrichment_kwargs: dict = {}, format_kwargs: dict = {}) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Perform ontology enrichment analysis pipeline."""

    dag, objanno, ns2assoc, gene2go, go2genes, slim_dag, ns2slim_assoc = load_ontology_data(ontology_data, **load_kwargs)
    ...
    oea_obj, oea_results_sig = ontology_enrichment(query_genes, bg_genes, **enrichment_kwargs)
    oea_obj_slim, oea_results_sig_slim = ontology_enrichment(query_genes, bg_genes, **slim_enrichment_kwargs)

    oea_results_sig_prt = format_ontology_enrichment_results("full", oea_results_sig, **format_kwargs)
    oea_results_sig_prt_slim = format_ontology_enrichment_results("slim", oea_results_sig_slim, **format_kwargs)

    return oea_results_sig_prt, oea_results_sig_prt_slim

# @st.cache_data
def stringdb_api_functions(output_format: STRING_OUTPUT_FORMAT = "xml", method: STRING_METHOD = "get_string_ids", params: dict = {}):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if "Something went wrong!" in response.text:
                raise RequestException("communication_error: Something went wrong! -- please contact STRING maintainers if the issue persists")
            break
        except RequestException as e:
            if attempt < MAX_RETRIES - 1:
                time.sleep(RETRY_DELAY)
            else:
                raise ValueError(f"Error performing STRING API functions: {e} after {MAX_RETRIES} retries")
...
def format_string_enrichment_results(enrichment_df: pd.DataFrame, query_genes: list[str], background_genes: list[str]) -> pd.DataFrame:
    ...
    enrichment_df["ratio_in_study"] = str(enrichment_df["study_count"]) + "/" + str(enrichment_df["study_n"])
    ...
    enrichment_df['namespace_order'] = enrichment_df['namespace'].map({v: i for i, v in enumerate(namespace_order)})
...
def stringdb_enrichment(query_genes, bg_genes):
    ...
    get_string_id_response = stringdb_api_functions(output_format="xml", method="get_string_ids", params=get_string_id_params)
    ...
    for ns, ns_results in enrichment_results.groupby("namespace", sort=False):
        chart = (
            ...
            .encode(
                ...
                alt.Y(
                    "term:N",
                    axis=alt.Axis(grid=True, labelLimit=500, title="Term", orient="right"),
                    sort=alt.EncodingSortField(
                        field="gene_ratio", order="descending"),
                ),
